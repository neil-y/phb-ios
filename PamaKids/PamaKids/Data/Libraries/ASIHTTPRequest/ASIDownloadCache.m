...
{
    [self clearCachedResponsesForStoragePolicy:cachePolicy withBasePath:nil];
}

- (void)clearCachedResponsesForStoragePolicy:(ASICacheStoragePolicy)storagePolicy withBasePath:(NSString*) basePath;
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSError *error = nil;
	NSMutableArray *cacheFiles = [[NSMutableArray alloc] initWithArray:[fileManager contentsOfDirectoryAtPath:path error:&error] copyItems:YES];
    ...
    {
        NSString * headPath = [basePath stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
        for (int i=0; i<[cacheFiles count]; i++)
        {
            NSString * sub = cacheFiles[i];
            if(![sub hasPrefix:headPath])
            {
                [cacheFiles removeObjectAtIndex:i];
                i--;
            }
        }
    }
...
- (void)clearCachedResponsesForStoragePolicy:(ASICacheStoragePolicy)storagePolicy withoutBasePath:(NSString*) basePath
{
    [[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
+ (NSString *)keyForURL:(NSURL *)url
{
    ...
    NSString * path = [url.path stringByReplacingOccurrencesOfString:@"/" withString:@"_"];
	return  [NSString stringWithFormat:@"%@_%@",path,[self md5:urlString]];
}
